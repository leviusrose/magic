...
| `25` NetworkDeath (対象敵) | その対象の DoT を消去 |
| `25` NetworkDeath (**自キャラ**) | **全 DoT クリア** (全滅・即死リセット) |
| `04` RemoveCombatant | 敵が despawn (wipe・撃破) されたら対象の DoT を消去 |
| `260` InCombat 1→0 | 戦闘終了で全 DoT クリア |
...
4. 動作確認したい時だけ `CONFIG.debug = true` にして Reload すると、上部バナーに
   `own:<ID>(<名前>)` が出る。自キャラが確定していれば購読 OK。
...
var CONFIG = {
  ...
  showStatus:   false,      // 左下に「● 自キャラ名」を表示 (切り分け時のみ true)
